...
class _SuccessStoriesScreenState extends State<SuccessStoriesScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          ?widget.type == "stories" ?customAppBar(
              appBarInd: 0,
              widget: SizedBox(),status: false,title: tr("success_stories")
            ...
            child: BlocBuilder<SuccessStoriesCubit, SuccessStoriesState>(
              builder: (context, state) {
                return context.read<SuccessStoriesCubit>().loading == true ?
                  ...
                  ListView.builder(
                itemCount: context.read<SuccessStoriesCubit>().successStoriesResponse!.data!.length,
                shrinkWrap: true,
                scrollDirection: Axis.vertical,
                addAutomaticKeepAlives: true,
                physics: ClampingScrollPhysics(),
                itemBuilder: (context,index)=> SuccessStoriesItem(data: context.read<SuccessStoriesCubit>()
                    .successStoriesResponse!.data![index],));
  },
),
          ),
        ],

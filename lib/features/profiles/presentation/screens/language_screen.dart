...
class _LanguageScreenState extends State<LanguageScreen> {
  ...
  Widget build(BuildContext context) {
   return Scaffold(
     appBar: customAppBar(
         appBarInd: 0,
         widget: SizedBox(),title: tr("language"),status: false,context: context),
           ...
           builder: (context, state) {
             return ListView.builder(
               ...
               itemBuilder: (context, index) {
                 return LanguageItem(item: context.read<LanguageCubit>().items[index], index: index,);
               },);},),
         Spacer(),
         BlocBuilder<LanguageCubit, LanguageState>(
           builder: (context, state) {
    return FadeColorButton(btnTitle: tr("save"),isPressed: false,buttonColor: primary
      ,onButtonPressed: (){
        if(context.read<LanguageCubit>().lang! == 'en'){
          di.helper.updateLocalHeader("en");
          msgKey.currentState!.showSnackBar(SnackBar(
              behavior: SnackBarBehavior.floating,
              content: Text("Language has been set to english",style: TextStyles.textStyleNormal13
                  .copyWith(color: white),textScaler: TextScaler.linear(1),)));
          context.setLocale(Locale(context.read<LanguageCubit>().lang!));
          context.pushNamedAndRemoveUntil(name: splash);
        }else {
          di.helper.updateLocalHeader("ar");
          msgKey.currentState!.showSnackBar(SnackBar(
              behavior: SnackBarBehavior.floating,
              content: Text("تـم تغيير اللغـه إلي العـربيـة",style: TextStyles.textStyleNormal13
                  .copyWith(color: white),textScaler: TextScaler.linear(1),)));
          context.setLocale(Locale(context.read<LanguageCubit>().lang!));
          context.pushNamedAndRemoveUntil(name: splash);
        }
